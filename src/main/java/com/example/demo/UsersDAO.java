...
public class UsersDAO {
	...
	public Users getById(String id) {
		Users users = (Users) ht.get(Users.class, id);
		return users;
	}
	
	public Users getByIdOrEmail(String id, String email) {
		Session session = ht.getSessionFactory().openSession();
		Query query = session.createQuery("SELECT u From  Users u WHERE u.userId=? OR u.email=?");
		query.setParameter(0, id);
		query.setParameter(1, email);
		List<Users> s = query.list();
		if (s == null || s.size() == 0) {
			return null;
		}
		return s.get(0);
