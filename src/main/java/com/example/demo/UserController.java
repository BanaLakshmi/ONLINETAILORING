...
public class UserController {
	...
	@RequestMapping("/ViewCompletedOrder")
	public ModelAndView ViewCompletedOrder (HttpSession session) {
		ModelAndView mv = new ModelAndView();
		mv.setViewName("ViewCompletedOrder");
		return mv;
	}
	...
	@RequestMapping("/ViewCompletedCustomerOrder")
	public ModelAndView ViewCompletedCustomerOrder (HttpSession session) {
		ModelAndView mv = new ModelAndView();
		mv.setViewName("ViewCompletedCustomerOrder");
		return mv;
	}
	...
	public ModelAndView addbillGeneration(BillGeneration billGeneration,HttpSession session) {
		Users user = (Users) session.getAttribute("user");
		ApplicationContext ctx = new ClassPathXmlApplicationContext("bean.xml");
		BillGenerationDao bGdao = (BillGenerationDao) ctx.getBean("bGdao");
		BillGeneration billGenerationDB = bGdao.getByorderId(billGeneration.getOrderId());
		ModelAndView mv = new ModelAndView();
		if (billGenerationDB == null) {
			BillGeneration billGeneration1 = bGdao.saveBillGeneration(billGeneration);
		if (billGeneration1 == null) {
			mv.setViewName("failure");
			
		} else 
			mv.setViewName("tailor");
			mv.addObject("message", "Bill is generated");
		}else {
			mv.setViewName("tailor");
			mv.addObject("message", "Bill already exist please try with a new one");
		}
		return mv;
	}
	...
	public ModelAndView CompletedOrdres (HttpSession session,String tailorId,String status)
	
	{
		...
		if (s == null) {
			...
			mv.setViewName("completedOrders");
			mv.addObject("s",s);
		}
	...
	@RequestMapping("/CustomerCompletedOrders")
	public ModelAndView CompletedCustomerOrdres (HttpSession session,String customerId,String status)
	
	{
		...
		if (s == null) {
			...
			mv.setViewName("CustomerCompletedOrders");
			mv.addObject("s",s);
		}
	...
	public ModelAndView saveHelp(Help help, HttpSession session, ModelMap model) {
		...
		help.setUserId((String) session.getAttribute("userId"));
	...
	public ModelAndView saveResolution(String resolution, HttpSession session, ModelMap map)
		...
		System.out.println(session.getAttribute("help").toString());
	...
	@RequestMapping("/ViewResolution")
	public ModelAndView viewResolution( HttpSession session, ModelMap map)
			throws NullPointerException {
		ApplicationContext ctx = new ClassPathXmlApplicationContext("bean.xml");
		HelpDAO hdao = (HelpDAO) ctx.getBean("Hdao");
		//Help help=(Help) session.getAttribute("help1");
		
		String uId=(String) session.getAttribute("userId");
		System.out.println(uId);
		
		List<Help>helpsList=
				hdao.getResolutionByUserId((uId));
		ModelAndView mv = new ModelAndView();
		if (helpsList.isEmpty()) {

			mv.setViewName("NoRequests");
			mv.addObject("message", "you didnt rise a requets yet...");

		} else {
			
			session.setAttribute("helpsList", helpsList);

			mv.setViewName("UserHelpRequests");
