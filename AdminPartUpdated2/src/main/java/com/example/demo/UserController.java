...
public class UserController {
	...
	@RequestMapping("/adminAddFeedbackQuestion")
	public ModelAndView adminAddFeedbackQuestion() {
		ModelAndView mv = new ModelAndView();
		mv.setViewName("AdminAddFeedbackQuestion");
		return mv;
	}
	...
	@RequestMapping("/saveFeedbackQuestion")
	public ModelAndView saveFeedbackQuestion(FeedbackQuestion feedbackQuestion,HttpSession session,ModelMap model) {
		ApplicationContext ctx = new ClassPathXmlApplicationContext("bean.xml");
		FeedbackQuestionDAO Fdao = (FeedbackQuestionDAO) ctx.getBean("Fdao");
		//System.out.println(feedbackQuestion.toString());
		FeedbackQuestion feedbackQuestion1 = Fdao.create(feedbackQuestion);
		...
		//session.setAttribute("feedbackQuestion1", feedbackQuestion);
		return mv;
	}
	...
	public ModelAndView viewResolution( HttpSession session, ModelMap map)
		...
		System.out.println(session.getAttribute("userId"));
		String resolution=hdao.getResolutionByUserId((String) session.getAttribute("userId"));
		System.out.println(resolution);
		session.setAttribute("resolution", resolution);
		//session.setAttribute("issue", help.getIssue());
		//session.setAttribute("resolution",help.getResolution());
		//int no=hdao.updateResolution(resolution,help.getRequestId());
		//System.out.println(help.getRequestId());
		ModelAndView mv = new ModelAndView();
		mv.setViewName("viewResolution");
