...
public class HelpDAO {
	...
	public String getResolutionByUserId(String userId)
	{
		Session session = ht.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query query=session.createSQLQuery("select h.resolution from Help h  where h.userId=?");
		System.out.println("user id is"+userId);
		query.setParameter(0, userId);
		String resolution=(String) query.uniqueResult();
